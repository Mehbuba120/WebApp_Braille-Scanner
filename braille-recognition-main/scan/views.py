...
class UserHistorylView(LoginRequiredMixin,ListView):
    model = Uploads

    def get(self, request: HttpRequest, *args: str, **kwargs: str):
        history=Uploads.objects.filter(user=request.user)
        return render(request, template_name='scan/history.html', context={'history': history})
